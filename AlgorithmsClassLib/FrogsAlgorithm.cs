...
public static class FrogsAlgorithm
{
    ...
    public static void RunFrogsAlg(
        in ProblemParams parameters, 
        ref int[] taskOrder,
        int numOfSubgroups = 10, 
        int subgroupQuantity = 10, 
        int memeplexCycles = 5, 
        int populationCycles = 10)
    {
        ...
        // Генерация популяции
        var population = Population.GeneratePopulation(parameters, parameters.NumOfTasks,
            numOfSubgroups * subgroupQuantity);

        // лучшая особь в популяции
        var bestIndividual = new int[parameters.NumOfTasks];
        // лучшие особи в мемплексах
        var bestMemeplexIndividual = new int[numOfSubgroups][];

        // Временный порядок выполнения работ
        var tmpOrder = new int[parameters.NumOfTasks];

        // Циклы алгоритма для популяции в целом
        for (var plnItr = 0; plnItr < populationCycles; plnItr++)
        {
            ...
            subgroups = new();
            for (var i = 0; i < numOfSubgroups; i++)
            {
                subgroups.Add(new());
                for (var j = 0; j < subgroupQuantity; j++)
                {
            ...
            for (var mpxItr = 0; mpxItr < memeplexCycles; mpxItr++)
            {
                ...
                for (var i = 0; i < numOfSubgroups; i++)
                {
                    ...
                    for (var ind = 0; ind < parameters.NumOfTasks; ind++)
                    {
                    ...
                    if (!parameters.CheckForBetterFitness(
                        subgroups[i][subgroupQuantity - 1], 
                        tmpOrder))
                    {
    ...
    public static void MoveTo(int numOfTasks, int[] worstFrog, in int[] leaderFrog)
    {
        ...
        for (var i = 0; i < numOfTasks; i++)
        {
            ...
            worstFrog[i] = (int)Math.Round(
                rand.NextDouble() * (leaderFrog[i] - worstFrog[i]) + worstFrog[i],
